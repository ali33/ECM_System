    ...
    public class RecoveryHelper
    {
        ...
        public void Backup()
        {
            ...
            foreach (ContentItem item in ViewerContainer.Items)
            {
                ...
                if (item.Children != null && item.Children.Count > 0)
                {
                    ...
                    List<ContentItem> docs = item.Children.Where(p => p.ItemType == ContentItemType.Document).ToList();
                    if (docs.Count > 0)
                    {
                        hasData = true;
            ...
            if (hasData)
            {
                string xml = UtilsSerializer.Serialize(recoveryInfo, _rootNode);
                    ...
                    if (batchType != null && batch.BatchData.CreatedBy == ViewerContainer.UserName)
                    {
                        ...
                        if (batch.Documents != null && batch.Documents.Count > 0)
                        {
                            foreach (RecoveryDoc doc in batch.Documents)
                            {
                                ...
                                var docType = batchType.DocumentTypes.FirstOrDefault(p => p.Id == doc.DocumentData.DocumentType.Id);
                                if (docType != null)
                                {
        ...
        private void RestoreWithNoBackupInfo()
        {
            ...
            if (files.Length > 0)
            {
                ...
                {
                    var pageItem = new ContentItem(file, null);
        ...
        private const string _rootNode = "Recovery";
    }
}
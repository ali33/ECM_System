...
namespace Ecm.DocViewer.Model
{
    ...
    {
        private readonly ResourceManager _resource = new ResourceManager("Ecm.DocViewer.Resources", Assembly.GetExecutingAssembly());
        ...
        public ContentItem(string filePath, string originalFileName)
        {
            ...
            PageData.FileBinaries = File.ReadAllBytes(filePath);
        ...
        public void SetDocumentData(DocumentModel document)
        {
            if (ItemType == ContentItemType.Document)
            {
                ...
                DocumentData.LinkDocuments.CollectionChanged += LinkDocuments_CollectionChanged;
        ...
        private void LinkDocuments_CollectionChanged(object sender, NotifyCollectionChangedEventArgs e)
        {
        ...
        public byte[] GetBinary(DocViewerMode mode)
        {
            if (mode == DocViewerMode.LightCapture || mode == DocViewerMode.Document)
            {
                if (!string.IsNullOrEmpty(FilePath))
                {
        ...
        private void SetupTitles()
        {
            if (ItemType == ContentItemType.Batch)
            {
                ...
                Title2 = _resource.GetString("uiCreatedOn") + " " + BatchData.CreatedOn.ToString(Properties.Resources.LongDateTimeFormat);
            ...
            else if (ItemType == ContentItemType.Document)
            {
                ...
                Title2 = _resource.GetString("uiPage") + " " + Children.Count;
            }
            else
            {
        ...
        private void ChildrenCollectionChanged(object sender, NotifyCollectionChangedEventArgs e)
        {
            ...
            if (e.Action == NotifyCollectionChangedAction.Add || e.Action == NotifyCollectionChangedAction.Replace)
            {
                foreach (ContentItem item in e.NewItems)
                {
                    ...
                    {
                        item.Image.SetPermission(ViewerContainer.PermissionManager.GetViewerPermission(item));
                    }
                }
            }
        ...
        private void CreatePageThumbnail()
        {
            ...
            if (PageData.FileType == FileTypeModel.Image)
            {
                if (string.IsNullOrEmpty(FilePath))
                {
                    Image = new CanvasElement(Binary, PageData, ViewerContainer.PermissionManager.GetViewerPermission(this), ViewerContainer);
                }
                else
                {
                    Image = new CanvasElement(FilePath, PageData, ViewerContainer.PermissionManager.GetViewerPermission(this), ViewerContainer);
                }
            ...
            Image = new CanvasElement(new Uri("pack://application:,,,/DocViewer;component/Resources/unknow-page.png"), ViewerContainer) { IsNonImagePreview = true };
        ...
        {
            return new ImageBrush(BitmapFrame.Create(new Uri("pack://application:,,,/DocViewer;component/Resources/" + fileName)));
        }

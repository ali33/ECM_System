...
namespace Ecm.CaptureAdmin.ViewModel
{
    public class ReadActionViewModel : ComponentViewModel, IDataErrorInfo
    {
        ...
        public ReadActionViewModel(ReadActionModel readActionModel, List<DocTypeModel> docTypes, BatchTypeModel batchType)
        {
        ...
        public string this[string columnName]
        {
            get
            {
                ...
                {
                    if (BarcodePositionInDoc <= 0 || BarcodePositionInDoc > 100)
                    {
        ...
        private void Initialize()
        {
            ...
            List<FieldModel> normalBatchIndexes = _batchType.Fields.Where(p => !p.IsSystemField).ToList();
            foreach (FieldModel field in normalBatchIndexes)
            {
        ...
        private void InitializeDocIndexes()
        {
            ...
            if (IsDocIndex && DocTypeId != Guid.Empty)
            {
                DocTypeModel docType = _docTypes.FirstOrDefault(p => p.Id == DocTypeId);
                if (docType != null)
                {
                    foreach (FieldModel field in docType.Fields)
                    {
        ...
        private bool ValidateIndexNames(out string errorMessage)
        {
            ...
            if (IsDocIndex)
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    {
                        FieldModel indexField = docType.Fields.FirstOrDefault(p => p.Name.Equals(fieldName, StringComparison.OrdinalIgnoreCase));
                        if (indexField == null)
                        {
            ...
            else
            {
                for (int i = 0; i < count; i++)
                {
                    ...
                    if (!string.IsNullOrEmpty(fieldName))
                    {
                        FieldModel field = _batchType.Fields.FirstOrDefault(p => p.Name.Equals(fieldName, StringComparison.OrdinalIgnoreCase));
                        if (field == null)
                        {
                            errorMessage = string.Format(_resource.GetString("uiInvalidFieldName"), fieldName);
                            return false;
        ...
        private List<CopyValueToFieldModel> ParseIndexNames()
        {
            ...
            if (IsDocIndex)
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    {
                        FieldModel field = docType.Fields.FirstOrDefault(p => p.Name.Equals(fieldName, StringComparison.OrdinalIgnoreCase));
                        result.Add(new CopyValueToFieldModel
                                       {

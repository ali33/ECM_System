    ...
    public class ExportFileHelper
    {
        ...
        public void SendMail()
        {
            try
            {
                ...
                {
                    var dialogViewer = new DialogViewer { Width = 500, Height = 280, Text = "Select pages for email" };
                        ...
                        else
                        {
                            ...
                            if (rangeSelector.SelectedPageIndexes != null)
                            {
                                foreach (int index in rangeSelector.SelectedPageIndexes)
                                {
                                    ...
                                    {
                                        files.Add(item.PageData.FileBinary);
                                    }
                                    else
                                    {
                                        byte[] tiff = ImageProcessing.ConvertToTiff(item.PageData.FileBinary, item.PageData.FileExtension);
                                        files.Add(tiff);
                                    }
                                }
                            }
        ...
        public void SaveFile()
        {
            try
            {
                if (ViewerContainer.OpeningItem.PageData.FileType == FileTypeModel.Image)
                {
                    var dialogViewer = new DialogViewer { Width = 500, Height = 280, Text = "Select pages for saving" };
                    var rangeSelector = new SendMailOptionView
                    {
                        Dialog = dialogViewer
                    };

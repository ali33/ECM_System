...
namespace Ecm.CaptureModel
{
    public class FieldModel : BaseDependencyProperty, IDataErrorInfo
    {
        ...
        [XmlIgnore]
        public Guid WorkingId = Guid.NewGuid();
        ...
        {
            var newObj = (FieldModel)base.OnClone();

            if (LookupInfo != null)
            {
                newObj.LookupInfo = (LookupInfoModel)LookupInfo.Clone();
            }
            if (Maps != null)
            {
                newObj.Maps = new ObservableCollection<LookupMapModel>(Maps.Select(h => (LookupMapModel)h.Clone()));
            }
            if (RuntimeLookupMaps != null)
            {
                newObj.RuntimeLookupMaps = RuntimeLookupMaps.Select(h => h).ToList();
            }
            if (Children != null)
            {
                newObj.Children = new ObservableCollection<TableColumnModel>(Children.Select(h => (TableColumnModel)h.Clone()));
            }
            if (DeletedMaps != null)
            {
                newObj.DeletedMaps = DeletedMaps.Select(h => (LookupMapModel)h.Clone()).ToList();
            }
            if (DeletedChildrenIds != null)
            {
                newObj.DeletedChildrenIds = DeletedChildrenIds.Select(h => h).ToList();
            }
            if (Picklists != null)
            {
                newObj.Picklists = new ObservableCollection<PicklistModel>(Picklists.Select(h => (PicklistModel)h.Clone()));
            }
            if (OCRTemplateZone != null)
            {
                newObj.OCRTemplateZone = (OCRTemplateZoneModel)OCRTemplateZone.Clone();
            }
            if (ErrorMessages != null)
            {
                newObj.ErrorMessages = ErrorMessages.ToDictionary(h => h.Key, h => h.Value);
            }

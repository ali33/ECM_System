...
namespace Ecm.CaptureCore
{
    ...
    {0}
            ...
            : base(loginUser)
        {
            ...
            //{
            //    userGroupIds = new UserGroupDao(primaryContext).GetById(LoginUser.Id);
            //}
        }
        ...
        private void ValidateDocFieldValue(DocumentFieldValue docFieldValue, DocumentFieldMetaData docFieldMetaData, string docName)
        {
            ...
            switch (docFieldMetaData.DataType)
            {
                ...
                case "DATE":
                    ...
                    if (DateTime.TryParseExact(value, new string[] { "yyyy-MM-dd", "MM/dd/yyyy", "dd/MM/yyyy", "d/M/yyyy", "M/d/yyyy", "yyyy-MM-dd HH:mm:ss.fff", "MM/dd/yyyy HH:mm:ss.fff", "dd/MM/yyyy HH:mm:ss.fff", "d/M/yyyy HH:mm:ss.fff", "M/d/yyyy HH:mm:ss.fff" }, null, DateTimeStyles.None, out tempDate))
                    {
        ...
        private void ValidateBatchFieldValue(BatchFieldValue batchFieldValue, BatchFieldMetaData batchFieldMetaData)
        {
                ...
                try
                {
                    ...
                    else
                    {
                        foreach (Batch batch in batchs)
                        {
                            ...
                            canApprove = permission.UserGroupPermissions.Any(p => userGroupIds.Contains(p.UserGroupId));
                ...
                try
                {
                    ...
                    if (approvedBatchs.Count > 0)
                    {
                        foreach (Batch approvedBatch in approvedBatchs)
                        {
                            ...
                            batchDao.UpdateByWorkflow(approvedBatch);
                            batchDao.UnRejectRecur(approvedBatch.Id);
                            ActionLogHelper.AddActionLog("Approved work item: " + approvedBatch.Id + " successfully",
                                            LoginUser, ActionName.ApprovedBatch, null, null, context);
                ...
                try
                {
                    ...
                    if (rejectItems.Count > 0)
                    {
                        foreach (Batch rejectedBatch in rejectItems)
                        {
                            ...
                            var dbBatch = batchDao.GetById(rejectedBatch.Id);
                            if (dbBatch == null)
                            {
                                throw new Exception("Work item is have no longer existed.");
                            }
                            // Check save batch is in the last active transaction id
                            if (dbBatch.TransactionId != rejectedBatch.TransactionId)
                            {
                                throw new InvalidTransactionIdException("Your batch transaction ID is different from last active transaction ID");
                            }
                ...
                try
                {
                    ...
                    if (rejectItems.Count > 0)
                    {
                        foreach (Batch rejectedBatch in rejectItems)
                        {
                            ...
                            batchDao.RejectRecur(rejectedBatch.Id);
            ...
            using (DapperContext context = new DapperContext(LoginUser.CaptureConnectionString))
            {
                ...
                if (LoginUser.IsAdmin)
                {
                    ...
                    foreach (var batch in batches)
                    {
                        ...
                        {
                            if (fieldMetas.ContainsKey(fieldValue.FieldId))
                            {
        ...
        private List<Batch> FilterSearchWorkItemByType(List<Batch> batches, BatchStatus searchType)
        {
            switch (searchType)
            {
                case BatchStatus.Error:
                    return batches.Where(h => !h.IsCompleted
                                              && h.HasError).ToList();

                case BatchStatus.InProcessing:
                    return batches.Where(h => !h.IsCompleted
                                              && !h.HasError
                                              && h.IsProcessing).ToList();

                case BatchStatus.Locked:
                    return batches.Where(h => !h.IsCompleted
                                              && !h.HasError
                                              && !h.IsProcessing
                                              && !string.IsNullOrWhiteSpace(h.LockedBy)).ToList();

                case BatchStatus.Reject:
                    return batches.Where(h => !h.IsCompleted
                                              && !h.HasError
                                              && !h.IsProcessing
                                              && string.IsNullOrWhiteSpace(h.LockedBy)
                                              && h.IsRejected).ToList();
        ...
        public WorkItemSearchResult RunAdvanceSearchWorkItem(Guid batchTypeId, SearchQuery query, int pageIndex)
        {
            ...
            using (DapperContext dataContext = new DapperContext(LoginUser.CaptureConnectionString))
            {
                ...
                if (LoginUser.IsAdmin)
                {
                    foreach (var item in finalBatchs)
                    {
                        item.BatchPermission = BatchPermission.GetAllowAll();
                    }
                }
                else
                {
                    ...
                    var customActivityDao = new CustomActivitySettingDao(dataContext);

                    foreach (var item in finalBatchs)
                    {
                        // Try batch human step id
                        Guid humanStepId;
                        if (!Guid.TryParse(item.BlockingBookmark, out humanStepId))
                        {
                            continue;
                        }
                        // Get custom activity and permission
                        var customActivity = customActivityDao.GetCustomActivitySetting(item.WorkflowDefinitionId,
                                                                                        humanStepId);
                        if (customActivity == null)
                        {
                            item.BatchPermission = new BatchPermission();
                            continue;
                        }
        ...
        private void UpdateBatch(Batch batch, BatchPermission permission, DapperContext context, bool fromMobile = false)
        {
            ...
            if (batch.IsRejected)
            {
                if (!canReject)
                {
                    throw new Exception("No permission reject item");
                }
            }
            #endregion
        ...
        private void UpdateDocument(Document document, BatchPermission permission, DapperContext context, bool fromMobile = false)
        {
            ...
            documentDao.UpdateRejectStatus(document.Id, document.IsRejected);
            foreach (Page page in document.Pages)
            {
                if (page.Id != Guid.Empty)
                {
                    ...
                    pageDao.UpdateRejectStatus(page.Id, page.IsRejected);
                        ...
                        if (fieldValue.TableFieldValue != null && fieldValue.TableFieldValue.Count > 0)
                        {
                            ...
                            foreach (var tableValue in fieldValue.TableFieldValue)
                            {
                                ...
                                if (tableValue.Field != null)
                                {
                                    if (tableValue.Field.DataTypeEnum == FieldDataType.Date)
                                    {
                                        tableValue.Value = string.Format("{0}", tableValue.Value);
                                        int index = tableValue.Value.IndexOf(" ");
                                        if (index != -1)
                                        {
                                            tableValue.Value = tableValue.Value.Substring(0, index);
                            ...
                            tableValueDao.DeleteIfNotInListId(document.Id, fieldValue.FieldId, keepTablesIds);
                        }
...
",
        ...
        private void CallWorkflow(Batch batch, bool isNewDocument, params object[] resumeData)
        {
            ...
            using (DapperContext dataContext = new DapperContext(LoginUser.CaptureConnectionString))
            {
                ...
                Controller wfCore = new Controller(runtimeData, workflow.DefinitionXML, ConnectionStringEncryptionHelper.GetDecryptpedConnectionString(LoginUser.CaptureConnectionString, "D4A88355-7148-4FF2-A626-151A40F57330"));
                ...
                else
                {
                    ...
                    wfCore.ResumeWorkflow(batch.WorkflowInstanceId, batch.BlockingBookmark);
                }
                                ...
                                out int rejectedBatchCount)
        {
            using (DapperContext captureContext = new DapperContext(LoginUser))
            {
                ...
                // Count item
                errorBatchCount = batches.Count(h => !h.IsCompleted
                                          && h.HasError);

                inProcessingBatchCount = batches.Count(h => !h.IsCompleted
                                              && !h.HasError
                                              && h.IsProcessing);

                lockedBatchCount = batches.Count(h => !h.IsCompleted
                                              && !h.HasError
                                              && !h.IsProcessing
                                              && !string.IsNullOrWhiteSpace(h.LockedBy));

                rejectedBatchCount = batches.Count(h => !h.IsCompleted
                                              && !h.HasError
                                              && !h.IsProcessing
                                              && string.IsNullOrWhiteSpace(h.LockedBy)
                                              && h.IsRejected);

                availableBatchCount = batches.Count(h => !h.IsCompleted
                                              && !h.HasError
                                              && !h.IsProcessing
                                              && string.IsNullOrWhiteSpace(h.LockedBy)
                                              && !h.IsRejected);
        ...
        public Guid GetTransactionId(Guid batchId)
        {
            ...
            {
                var batchDao = new BatchDao(context);
                var batch = batchDao.GetById(batchId);

...
namespace ArchiveMVC5.Utility
{
    ...
    public class MailHelper
    {
        ...
        #endregion
        public MailHelper(string _username, string _password, string host, int port)
        {
            ConfigDefaultSMTP(_username, _password, host, port);
        }
        public MailHelper()
        {
            
        }
        private void ConfigDefaultSMTP(string user,string pass, string host, int port)
        {
            try
            {
                smtp = new SmtpClient

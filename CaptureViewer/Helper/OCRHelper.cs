...
namespace Ecm.CaptureViewer.Helper
{
    public class OCRHelper
    {
        ...
        public OCRHelper(ViewerContainer viewerContainer)
        {
            ...
            Stream ocrData = Assembly.GetAssembly(typeof(PanToolHelper)).GetManifestResourceStream("Ecm.CaptureViewer.OCRData.tessdata.zip");
            ...
            if (fieldValue != null)
            {
                switch (fieldValue.Field.DataType)
                {
                    case FieldDataType.String:
                        var tempValue = string.Format("{0}", value);
                        if (tempValue.Length > fieldValue.Field.MaxLength)
                        {
                            fieldValue.Value = tempValue.Substring(0, fieldValue.Field.MaxLength);
                        }
                        else
                        {

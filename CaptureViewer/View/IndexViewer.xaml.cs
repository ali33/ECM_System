...
namespace Ecm.CaptureViewer
{
    public partial class IndexViewer : UserControl
    {
        ...
        public static readonly DependencyProperty SelectedTableFieldValueProperty =
            DependencyProperty.Register("SelectedTableFieldValue", typeof(FieldValueModel), typeof(IndexViewer));

        public static readonly DependencyProperty IsShowTableDetailProperty =
            DependencyProperty.Register("IsShowTableDetail", typeof(bool), typeof(IndexViewer));
        ...
        public void UpdateTitle()
        {
            ...
            if (FieldValues.Count > 0)
            {
                ...
                {
                    lblIndexTitle.Text = resource.GetString("uiDocIndex");
                ...
                {
                    lblIndexTitle.Text = resource.GetString("uiBatchIndex");
                    lblTitle1.Text = ViewerContainer.ThumbnailCommandManager.IndexedItems[0].BatchData.BatchType.Name;
                    lblTitle2.Text = ViewerContainer.ThumbnailCommandManager.IndexedItems[0].BatchItem.Children.Count + " docType(s)";
                }
            }
            else
            {
        ...
        public FieldValueModel SelectedTableFieldValue
        {
            get { return GetValue(SelectedTableFieldValueProperty) as FieldValueModel; }
            set { SetValue(SelectedTableFieldValueProperty, value); }
        }

        public bool IsShowTableDetail
        {
            get { return (bool)GetValue(IsShowTableDetailProperty); }
            set { SetValue(IsShowTableDetailProperty, value); }
        }
        ...
        {
            btnNextDoc.Command = ViewerContainer.ThumbnailCommandManager.IndexNextDocumentCommand;
            btnPrevDoc.Command = ViewerContainer.ThumbnailCommandManager.IndexPreviousDocumentCommand;
        }
        ...
        private void IndexEditorKeyDown(object sender, KeyEventArgs e)
        {
            if (e.Key == Key.Tab)
            {
                ...
                if (control != null)
                {
                    ...
                    {
                        ViewerContainer.ThumbnailCommandManager.IndexPreviousDocumentCommand.Execute(null, btnPrevDoc);
                    }

                    if (e.KeyboardDevice.Modifiers == ModifierKeys.None && position == FieldValues.Count - 1)
                    {
                        ViewerContainer.ThumbnailCommandManager.IndexNextDocumentCommand.Execute(null, btnNextDoc);
                    }
                }
            }
        }
            ...
            if (editorControl != null && editorControl.Tag != null)
            {
                ...
                if (fieldValue != null && fieldValue.SnippetImage != null)
                {
                    ...
                    if (ViewerContainer.OpeningContainerItem != null &&
                        ...
                        ViewerContainer.ThumbnailCommandManager.IndexedItems.Any(p => p == ViewerContainer.OpeningContainerItem) && fieldValue.Field.OCRTemplateZone != null)
                    {
        ...
        private void AutoCompleteBoxLoaded(object sender, RoutedEventArgs e)
        {
            ...
            if (at != null)
            {
                ...
                if (fieldValue != null)
                {
                    var maps = fieldValue.Field.Maps ?? new ObservableCollection<LookupMapModel>();
                    DataTemplate template = GetLookupDataTemplate(maps.Where(p => !string.IsNullOrEmpty(p.DataColumn)).Select(p => p.Name).ToList());
        ...
        private void AutoCompleteBoxPreviewKeyDown(object sender, KeyEventArgs e)
        {
            ...
            {
                var index = at.DataContext as DocumentFieldValueModel;
        ...
        {
            //FieldValueModel fieldValueModel = (sender as StackPanel).DataContext as FieldValueModel;
            //FieldValueModel editFieldValue = FieldValues.SingleOrDefault(p => p.Field.Id == fieldValueModel.Field.Id);

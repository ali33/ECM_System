...
namespace Ecm.Audit.ViewModel
{
    public class ActionLogViewModel : ComponentViewModel
    {
        ...
        {            
            _numberRow = new SettingProvider().GetSettings().MaxSearchRows;
            BuildSearchPane();
            if (_numberRow == 0)
            {
                _numberRow = 100;
        ...
        void RunGlobalSearch(object sender, DoWorkEventArgs e)
        {
            ...
            try
            {
                var actionLogs = _actionLogProvider.SearchActionLog(Expression, _currentIndex, _numberRow, out _totalRow);
                ...
                {
                    ActionLogs = new ObservableCollection<ActionLogModel>(actionLogs);
                }
            }
            catch (Exception ex)
            {
        ...
        {
            return ActionLogs != null && ActionLog != null && (ActionLog.IsSelected || ActionLogs.Any(a => a.IsSelected)) && AllowedDeleteLog;
        }

        private void DeleteActionLog()
        {
            if (DialogService.ShowTwoStateConfirmDialog(Resources.uiConfirmDelete) == DialogServiceResult.No)
            {
                return;
            }
        ...
        {
            ActionLogs.Clear();
            Expression = ProcessHelper.BuildSearchExpression2(Searchs);
        ...
        {
            AllowedDeleteLog = _permissionProvider.GetAuditPermissionByUser(LoginViewModel.LoginUser).AllowedDeleteLog;
            GetActionLogs(_currentIndex);
        }
        ...
        private ObservableCollection<ActionLogModel> GetActionLogs(int index)
        {
            try
            {
                ...
                return ActionLogs = new ObservableCollection<ActionLogModel>(actionLogs);
            }
            catch(Exception ex)
            {
                ProcessHelper.ProcessException(ex);
